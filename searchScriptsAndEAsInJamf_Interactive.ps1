...
Function searchScripts {
    ...
    $script:countScripts = $allScripts.scripts.size
...
Function searchEAs {
    ...
    $script:countEAs = $allEAs.computer_extension_attributes.size
...
Write-Host "1. Scripts only"
Write-Host "2. Extension Attributes only"
Write-Host "3. Both"
Write-Host "Please enter your choice: " -NoNewline
$scriptOrEa = Read-Host

If (($scriptOrEa) -eq "1") {
    searchScripts
    $countEAs = 0
} ElseIf (($scriptOrEa) -eq "2") {
    searchEAs
    $countScripts = 0
} ElseIf (($scriptOrEa) -eq "3") {
    searchScripts
    searchEAs
} Else {
    Write-Host "Bye"
